...
write_to_csv() {
    ...
    if [[ -n "$course_name" && -n "$start_date" && -n "$day_of_class" && -n "$start_time" && -n "$end_date" && -n "$end_time" && -n "$location" ]]; then
        ...
        # Format data for CSV
        current_date=$(date -j -f "%d-%b-%Y" "$start_date" "+%Y-%m-%d")
        end_date_epoch=$(date -j -f "%d-%b-%Y" "$end_date" "+%s")

        # Loop to add weekly events until the end date
        while [[ $(date -j -f "%Y-%m-%d" "$current_date" "+%s") -le $end_date_epoch ]]; do
            echo "$course_name,$current_date,$start_time,$current_date,$end_time,$description,$location" >> "$OUTPUT_FILE"
            current_date=$(get_next_week_date_mac "$current_date")  # Increment by one week
        done
    fi
}
